...
# 🔴 Fingerprint of the SHIPPED flow_model_mf6 archive -- the calibration every frozen
# case-study golden was built from. Bump together with the archive and the goldens.
#
# Why this exists (2026-08-28): the version gate above is NOT sufficient. It admits any
# workspace stamped `archive_version >= MIN`, and `stamp_flow_manifest()` deliberately
# lets a legitimate LOCAL 05f output stamp itself current so it is not clobbered. A
# workspace regenerated by 05f (RUN_PEST_LOCALLY=True) therefore passes the version gate
# with DIFFERENT CONTENT, and `ensure_flow_model` served it to every later caller
# silently and indefinitely.
#
# That happened on the JupyterHub: local `botm` drifted (07eb1dc11896f126 vs the
# canonical 145effba3ba79799), which propagated into the refined `botm` and, through
# `strt = max(strt, botm + 0.01)`, into `strt`. All NINE case-study goldens then failed
# their grid pin, and the student flow case study CRASHED -- with an error naming the
# grid while the cause was a data file. Diagnosis cost two days and three refuted
# hypotheses. See DOCUMENTATION/contracts/evidence/s3b/ROOT_CAUSE_MOTHER_MODEL_DRIFT.md.
CANONICAL_FLOW_FINGERPRINT = "6a9e27c455dcbb66"

#: Set to "1" to silence the drift warning (deliberate local calibration experiments).
_SKIP_FINGERPRINT_ENV = "AGM_ALLOW_LOCAL_FLOW_MODEL"
...
def _check_flow_model_is_canonical(ws: Path, *, require_canonical: bool,
                                   freshly_downloaded: bool = False) -> None:
    """Warn (or raise) when the resolved workspace is not the shipped calibration."""
    report = verify_flow_model(ws)
    if report["fingerprint"] is None or report["is_canonical"]:
        return
    if freshly_downloaded:
        raise FileNotFoundError(
            "The DOWNLOADED flow_model_mf6 archive does not match "
            f"CANONICAL_FLOW_FINGERPRINT.\n  archive     : {report['fingerprint']}\n"
            f"  expected    : {report['canonical']}\n"
            "Either the shipped archive was regenerated without updating "
            "CANONICAL_FLOW_FINGERPRINT, or the constant is wrong. Every frozen "
            "case-study golden was built on the expected calibration, so they are all "
            "suspect until this is reconciled."
        )
    msg = _flow_model_drift_message(ws, report)
    if require_canonical:
        raise FileNotFoundError(msg)
    if os.environ.get(_SKIP_FINGERPRINT_ENV) != "1":
        warnings.warn(msg, stacklevel=3)


def verify_flow_model(sim_ws: Union[str, Path]) -> Dict[str, Any]:
    """Is this workspace the SHIPPED calibrated flow model, or a local variant?

    Returns ``{fingerprint, canonical, is_canonical, manifest_fingerprint,
    manifest_consistent}``. Two independent questions, kept apart because they fail for
    different reasons:

    * ``manifest_consistent`` -- do the files on disk match the fingerprint recorded in
      their own ``MANIFEST_flow.json``? False means a partial extraction or an edit
      without a re-stamp.
    * ``is_canonical`` -- do the files match the SHIPPED archive
      (:data:`CANONICAL_FLOW_FINGERPRINT`)? False means a legitimate-but-different local
      calibration, which every golden-pinned build will reject.

    Never raises for a mismatch -- it reports. :func:`ensure_flow_model` decides.
    """
    ws = Path(sim_ws)
    try:
        actual = flow_model_fingerprint(ws)
    except FileNotFoundError:
        actual = None
    manifest_fp = None
    mpath = ws / _FLOW_MANIFEST_NAME
    if mpath.exists():
        try:
            manifest_fp = json.loads(mpath.read_text()).get("flow_fingerprint")
        except (ValueError, OSError):
            manifest_fp = None
    return {
        "fingerprint": actual,
        "canonical": CANONICAL_FLOW_FINGERPRINT,
        "is_canonical": actual == CANONICAL_FLOW_FINGERPRINT,
        "manifest_fingerprint": manifest_fp,
        "manifest_consistent": (manifest_fp is None or actual is None
                                or manifest_fp == actual),
    }


def _flow_model_drift_message(ws: Path, report: Dict[str, Any]) -> str:
    """The message that should have existed on 2026-08-26, and would have saved two days."""
    return (
        "The calibrated flow model at {ws} is NOT the shipped one.\n"
        "  fingerprint here : {here}\n"
        "  shipped archive  : {canon}\n"
        "\n"
        ...
        "\n"
        "To restore the shipped model:\n"
        "    mv {ws} {ws}.local-backup\n"
        "    # then re-run; ensure_flow_model() downloads the shipped archive\n"
        "\n"
        "To keep a local calibration deliberately, set {env}=1 (golden-pinned builds "
        "will still refuse it)."
    ).format(ws=ws, here=report["fingerprint"], canon=report["canonical"],
             env=_SKIP_FINGERPRINT_ENV)
...
def ensure_flow_model(sim_path: Optional[Union[str, Path]] = None, *,
                      require_canonical: bool = False) -> Path:
    ...
    if nam_path.exists():
        if _flow_manifest_version(sim_path) >= FLOW_MODEL_MIN_VERSION:
            # 🔴 The version gate is not enough. It admits any workspace stamped
            # current, INCLUDING one 05f regenerated in place with different content.
            # Verify the CONTENT before handing it to callers (2026-08-28).
            _check_flow_model_is_canonical(sim_path, require_canonical=require_canonical)
            return sim_path
        warnings.warn(
    ...
    # A freshly downloaded archive that is not canonical means the SHIPPED artifact and
    # CANONICAL_FLOW_FINGERPRINT disagree -- the constant or the archive is out of date,
    # and every golden is now suspect. Always loud, never silent.
    _check_flow_model_is_canonical(sim_path, require_canonical=require_canonical,
