...
- **B** is wrong: the spill sits directly on the spill→well axis, and the capture zone's **bisected half-width at the spill transect is ≈ 53 m** — the 10 m footprint is nowhere near escaping it.
- **C** is wrong twice over. (i) The footprint does not straddle anything: a 10 m disc centred on the axis sits ~53 m inside the boundary, so *nothing* in it escapes. (ii) And do not carry away "the probe shows roughly half" either — the wider 120 m probe returns a fraction of **≈0.49**, which *looks* like half but is a coincidence, not a physical split: it is a property of **that probe disc** ("the fraction of a 120 m disc that happens to lie inside the capture zone"), a disc that even reaches ~30 m *downgradient* of the well, where capture is impossible. The scalar is an artefact of where we chose to release; the captured/escaped **scatter** and the bisected half-width are the real results.
