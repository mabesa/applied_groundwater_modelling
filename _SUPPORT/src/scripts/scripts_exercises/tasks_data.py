...
04t already built and verified this model and read the compliance breakthrough: the conservative spill **reaches the extraction (compliance) well and exceeds** the 1.0 mg/L threshold (peak ≈ 5.1 mg/L at ~day 41). Taking that verdict as given, the applications below vary three levers:
...
$$\Delta t_{max} = \frac{\Delta s}{v} = \frac{10.8}{28.3} \approx 0.38 \text{ d}$$
...
With a mean water velocity of order a few metres per day over ~90 m, an arrival on the scale of **weeks-to-a-couple-of-months** is expected, and because a large finite mass is released the peak comfortably clears the stated threshold. When you run the next cell you should see a **peak of ≈ 5.1 mg/L at ~day 41** — an early, clear exceedance, matching prediction A. (Options B and C under-estimate how fast a forced-gradient doublet delivers a nearby upgradient spill.)
<br>
""",
...
- **Reactions remove mass.** Sorption (retardation R) and first-order decay (λ) both take mass out of the mobile aqueous phase — decay destroys it, sorption parks it on the solid. Strong enough reactions therefore lower the *peak concentration*, and enough attenuation crosses below the threshold and flips the verdict. (At the specific values Rung A explores — R = 2, a 30-day half-life — the peak only drops to ≈3.2 / ≈2.95 mg/L, so the exceedance survives; but the *type* of lever is the one that can eliminate it.)
- **Dispersivity reshapes; it does not remove mass (B is C's overstatement).** Doubling α_L (and α_T with it) spreads the finite pulse in time and across the corridor — earlier toe, broader front, longer tail, and a lower peak by dilution — but mass is conserved, and here the peak only eases from ≈5.1 to ≈4.3 mg/L. It reshapes the breakthrough far more than it moves the verdict, so "always dilutes below any threshold" (option C) is an overstatement.
...
- **R = 2, sorption-only**: peak ≈ **3.2 mg/L at ≈ day 59** (vs. the conservative ≈5.1 mg/L @ day 41) — later AND lower.

Rung A also runs a **separate** decay-only variant ($\lambda = \ln 2/30$ d, R = 1, no sorption) — this checkpoint is not about that curve, but the contrast is the key teaching point: decay-only gives peak ≈ **2.95 mg/L**, at **essentially the same arrival time** as the conservative run. In other words, **retardation delays arrival** (it slows the plume itself), while **decay does not** (it only removes mass in transit, it does not slow anything) — both lower the peak, but only retardation shifts *when* the peak shows up.
...
Running Rung B confirms it: peak ≈ **4.3 mg/L at ≈ day 39** (vs. the α_L=10 m keystone's ≈5.1 mg/L @ day 41) — earlier and lower, with both grid Péclet numbers (Pe_L and Pe_T) halved (a larger dispersivity is easier to resolve on the same grid).
...
- **B** is wrong: the spill sits directly on the spill→well axis, and the capture zone's **bisected half-width at the spill transect is ≈ 76 m** — the 10 m footprint is nowhere near escaping it.
- **C** is wrong twice over. (i) The footprint does not straddle anything: a 10 m disc centred on the axis sits ~76 m inside the boundary, so *nothing* in it escapes. (ii) And do not carry away "the probe shows roughly half" either — the wider 120 m probe returns a fraction of **≈0.68**, not ~0.5, and that number is not a physical split at all: it is a property of **that probe disc** ("the fraction of a 120 m disc that happens to lie inside the capture zone"), a disc that even reaches ~30 m *downgradient* of the well, where capture is impossible. The scalar is an artefact of where we chose to release; the captured/escaped **scatter** and the bisected half-width are the real results.
