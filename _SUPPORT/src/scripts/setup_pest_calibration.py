...
* **Pilot points** (~25) distributed across the active model domain,
  log-transformed K with bounds appropriate for a gravel aquifer (1–100 m/d).
...
* **Regularisation** — preferred-value (Tikhonov) regularisation so that K
  stays close to a prior estimate where data is sparse.
...
def build_pest_setup(
    ...
    * A forward-run Python script that:
    ...
    pt_weight : float
        Weight for the prior-information equation.
...
def _write_pst_file(path, n_pp, k_initial, k_lower, k_upper,
                    ...
                    tpl_name, ins_name, par_file, obs_file,
                    python_exe="python"):
    """Write a minimal PEST control file."""
    n_obs = len(obs_info)
    n_prior = 1  # one PI equation for PT K
    ...
    n_pargp = 1   # one parameter group: hk_grp
    n_obsgp = 2   # observation groups: heads, prior_info
    lines.append(f" {n_pp}  {n_obs}  {n_pargp}  {n_prior}  {n_obsgp}\n")
    lines.append("  1  1  single  point  1\n")        # NTPLFLE NINSFLE PRECIS DPOINT NUMCOM
    ...
    lines.append("0.01\n")                              # PHIREDSWH
    lines.append("3  0.01  3  3  0.01  3\n")           # NOPTMAX PHIREDSTP NPHISTP NPHINORED RELPARSTP NRELPAR
    lines.append("0  0  0\n")                           # ICOV ICOR IEIG
