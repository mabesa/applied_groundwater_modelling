...
def plot_cross_section(modelgrid, row=None, col=None, mf=None, ibound=None, river_data=None, figsize=(16, 12), show_top_view=True):
    ...
    Example usage:
    -----------
    # For a row cross-section (east-west):
    fig, axes = plot_cross_section(modelgrid, row=26, mf=mf, ibound=ibound, river_data=river_data)

    # For a column cross-section (north-south):
    fig, axes = plot_cross_section(modelgrid, col=30, mf=mf, ibound=ibound, river_data=river_data)

    # For the middle row (your original example):
    # middle_row = modelgrid.nrow // 2
    fig, axes = plot_cross_section(modelgrid, row=middle_row, mf=mf, ibound=ibound, river_data=river_data)
    plt.show()

    Usage Notes:
    -----------
    - Specify either `row` or `col`, not both.
    - If neither is specified, a ValueError will be raised.
    - The function will raise an error if the specified row or column is out of bounds.
    - The function will plot the model grid, layers, and river cells if available.
    - The top view will show the model domain with the cross-section line highlighted.
    """
    import matplotlib.pyplot as plt
    import numpy as np
        ...
        if ibound is not None:
            ...
            if np.any(inactive_mask):
                ...
                if len(inactive_rows) > 0:
                    ...
                    ax_top.scatter(inactive_x, inactive_y, c='red', s=10, alpha=0.5, 
    ...
    if mf is not None:
        if section_type == "row":
            xs_top = flopy.plot.PlotCrossSection(model=mf, line={'row': row}, ax=ax1)
        else:
            xs_top = flopy.plot.PlotCrossSection(model=mf, line={'column': col}, ax=ax1)
        
        # Plot the grid
        patches = xs_top.plot_grid(alpha=0.5, linewidth=0.5, color='black')
        
        # Plot the model layers (top and bottom)
        patches = xs_top.plot_surface(modelgrid.top, color='brown', alpha=0.8, linewidth=2, label='Model Top (DEM)')
        patches = xs_top.plot_surface(modelgrid.botm[0], color='gray', alpha=0.8, linewidth=2, label='Model Bottom')
        
        # Fill the aquifer - use fallback approach to avoid color conflicts
        try:
            xs_top.plot_fill_between(modelgrid.top, modelgrid.botm[0], color='lightblue', alpha=0.3, label='Aquifer')
        except (ValueError, TypeError) as e:
            print(f"FloPy fill method failed ({e}), using matplotlib fallback...")
            
            # Get coordinates and elevations
            if section_type == "row":
                x_coords = modelgrid.xcellcenters[row, :]
                top_elevs = modelgrid.top[row, :]
                bottom_elevs = modelgrid.botm[0, row, :]
            else:
                x_coords = modelgrid.ycellcenters[:, col]
                top_elevs = modelgrid.top[:, col]
                bottom_elevs = modelgrid.botm[0, :, col]
            
            ax1.fill_between(x_coords, bottom_elevs, top_elevs, 
                           color='lightblue', alpha=0.3, label='Aquifer')
        
        # Plot river cells if they exist along this section
        if river_data is not None and len(river_data) > 0:
            if section_type == "row":
                river_cells_in_section = river_data[river_data[:, 1] == row]  # row index is in column 1
                coord_idx = 2  # column index
            else:
                river_cells_in_section = river_data[river_data[:, 2] == col]  # col index is in column 2
                coord_idx = 1  # row index
            
            if len(river_cells_in_section) > 0:
                for i, river_cell in enumerate(river_cells_in_section):
                    cell_idx = int(river_cell[coord_idx])
                    river_stage = river_cell[3]  # river stage
                    river_bottom = river_cell[5]  # river bottom
                    
                    # Get x-coordinate of the cell
                    if section_type == "row":
                        x_coord = modelgrid.xcellcenters[row, cell_idx]
                    else:
                        x_coord = modelgrid.ycellcenters[cell_idx, col]
                    
                    # Plot river stage and bottom (no labels for plot 1)
                    ax1.scatter(x_coord, river_stage, color='blue', s=50, marker='o', zorder=10)
                    ax1.scatter(x_coord, river_bottom, color='darkblue', s=50, marker='s', zorder=10)
        
        ax1.set_title(f'Cross-Section: Model Grid and Layers ({section_type.title()} {section_num})')
        ax1.set_xlabel('Distance along cross-section (m)')
        ax1.set_ylabel('Elevation (m a.s.l.)')
        # Remove legend from plot 1
        ax1.grid(True, alpha=0.3)
    
    # Plot 2: Detailed elevation profile
    if section_type == "row":
        x_coords = modelgrid.xcellcenters[row, :] / 1000  # Convert to km
        top_elevs = modelgrid.top[row, :]
        bottom_elevs = modelgrid.botm[0, row, :]
        xlabel = 'Distance (km)'
    else:
        x_coords = modelgrid.ycellcenters[:, col] / 1000  # Convert to km
        top_elevs = modelgrid.top[:, col]
        bottom_elevs = modelgrid.botm[0, :, col]
        xlabel = 'Distance (km)'
    
    thickness = top_elevs - bottom_elevs
    
    # Plot elevation profile
    ax2.fill_between(x_coords, bottom_elevs, top_elevs, color='lightblue', alpha=0.3, label='Aquifer')
    ax2.plot(x_coords, top_elevs, 'brown', linewidth=2, label='Model Top (DEM)')
    ax2.plot(x_coords, bottom_elevs, 'gray', linewidth=2, label='Model Bottom')
    
    # Plot river information if present
    if river_data is not None and len(river_data) > 0:
        if section_type == "row":
            river_cells_in_section = river_data[river_data[:, 1] == row]
            coord_idx = 2
        else:
            river_cells_in_section = river_data[river_data[:, 2] == col]
            coord_idx = 1
        ...
        if len(river_cells_in_section) > 0:
            for i, river_cell in enumerate(river_cells_in_section):
                ...
                if section_type == "row":
                    x_km = modelgrid.xcellcenters[row, cell_idx] / 1000
                else:
                    x_km = modelgrid.ycellcenters[cell_idx, col] / 1000
                
                # Plot water column
                ax2.fill_between([x_km-0.025, x_km+0.025], [river_bottom, river_bottom], 
                               [river_stage, river_stage], color='blue', alpha=0.7)
                
                if i == 0:
                    ax2.plot([x_km-0.025, x_km+0.025], [river_stage, river_stage], 
                            'blue', linewidth=3, label='River Stage')
                    ax2.plot([x_km-0.025, x_km+0.025], [river_bottom, river_bottom], 
                            'darkblue', linewidth=3, label='River Bottom')
    ...
    if ibound is not None:
        ...
        inactive_x = x_coords[~active_mask]
        inactive_top = top_elevs[~active_mask]
        if len(inactive_x) > 0:
            ax2.scatter(inactive_x, inactive_top, color='red', s=20, alpha=0.5, 
                       label='Inactive Cells', marker='x')
    else:
        active_mask = np.ones(len(x_coords), dtype=bool)  # All active if ibound not defined
    ...
    stats_text += f"Length: {(x_coords.max() - x_coords.min()):.1f} km\n"
    stats_text += f"Elevation range: {top_elevs.min():.1f} - {top_elevs.max():.1f} m\n"
    ...
    print(f"Cross-section length: {(x_coords.max() - x_coords.min()):.1f} km")
    print(f"Elevation range: {top_elevs.min():.1f} to {top_elevs.max():.1f} m a.s.l.")
